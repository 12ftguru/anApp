...
class ExtDirect_API {
    ...
    private $_publicAttribute = '@public';
    ...
    public function getState() {
        return array(
            ...
            'publicAttribute' => $this->_publicAttribute,
		...
		$rDoc = $rClass->getDocComment();
                foreach($rMethods as $rMethod) {
                    if(
                        ...
                        $isPublic = !!preg_match('/'.$this->_publicAttribute.'/', $doc);
                        $isRemote = !!preg_match('/' . $this->_remoteAttribute . '/', $doc);
                        if (($this->isPublic && $isPublic) || !$this->isPublic) {
                        if($isRemote) {
                            $method = array(
